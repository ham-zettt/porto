const designTools = [
	{
		name: "Inkscape",
		icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/inkscape/inkscape-original.svg",
	},
	{
		name: "Photoshop",
		icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/photoshop/photoshop-original.svg",
	},
	{
		name: "Canva",
		icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/canva/canva-original.svg",
	}, // Use a direct link to Canva logo
	{
		name: "Figma",
		icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg",
	},
];

const projectsDesign = [
	{
		title: "35th Central Group",
		image: "/images/design/centralgroup.png",
		tags: [{ name: "eco" }, { name: "elegant" }],
	},
	{
		title: "Astrea-ASIC",
		image: "/images/design/asic.png",
		tags: [{ name: "monochrome" }, { name: "fintage" }],
	},
	{
		title: "Blue Cotton",
		image: "/images/design/bluecotton.png",
		tags: [{ name: "elegant" }, { name: "modern" }],
	},
	{
		title: "LC Project",
		image: "/images/design/lc.png",
		tags: [{ name: "typography" }, { name: "monochrome" }],
	},
	{
		title: "FID Channel",
		image: "/images/design/fid.png",
		tags: [{ name: "geometry" }, { name: "minimalist" }],
	},
	{
		title: "Y-Getion",
		image: "/images/design/ygetion.png",
		tags: [{ name: "unity" }, { name: "solid" }],
	},
	{
		title: "VTMA E-Sport",
		image: "/images/design/vtma.png",
		tags: [{ name: "futuristic" }, { name: "shiny" }],
	},
];
